!++jtb
! Defining IMSTUPID basically shuts off ice-phase. This makes diagnosing macrophysical
! aspects of microphysics much easier for someone who is stupid. 11/12/15 
!--jtb
!#define IMSTUPID
...
MODULE MODULE_MP_MORR_TWO_MOMENT
   !! USE     module_wrf_error !! it appears nothing from this module is used here - JTB 10/8/12
...
!++jtb
!PCE 1D cloud USES
  use ce1_consts, ONLY: CP, G=>GRAV, R => RDRY, RV => RVIR, EP_2
  use ce1_diags, ONLY: MORR_UPHYS_DIAGS
  use ce1_types, ONLY: PCEPHYS
!--jtb
...
! AEROSOL PARAMETERS FOR IACT = 2
     ...
     REAL, PRIVATE ::      RHOAer  ! AEROSOL BULK DENSITY (KG/M3)
...
! PARAMETERS CURRENTLY SET FOR AMMONIUM SULFATE
         ...
         MA = 0.0284
         RR = 8.3187
         BACT = VI*OSM*EPSM*MW*RHOAer/(MAP*RHOW)
...
write(*,*) " GOT through Morrison MP init!! "
...
!, effcs, effis
   ...
   REAL, DIMENSION(kts:kte) ::                                                                & 
                            ...
                            EFFC1D, EFFI1D, EFFS1D, EFFR1D,DZ1D, AR1D,RHO1D,  &
   ! HM ADD GRAUPEL
...
!++jtb, not really used here since I am using deeper intrfc
  integer :: IPC
!--jtb
! wrf-chem
...
!++jtb Add PCE index for diagnostics
                                  IPC,AR1D, RHO1D &
!--jtb
                       )
...
!++jtb
      ...
      REAL, DIMENSION(KTS:KTE) ::  AR3D               ! AREA OF CLOUD COLUMN (Units are arbitrary)
      REAL, DIMENSION(KTS:KTE) ::  RHOCL              ! PCE LAYER DENSITY (KG M-3)
!--jtb
...
! SIZE PARAMETER VARIABLES
     ...
     REAL, DIMENSION(KTS:KTE) :: T3D_IN        ! STORAGE ARRAY FOR INCOMING TEMPERATURE
     REAL, DIMENSION(KTS:KTE) :: T3D_TT        ! STORAGE ARRAY FOR TEMPERATURE TENDENCY
     REAL, DIMENSION(KTS:KTE) :: QV3D_IN       ! STORAGE ARRAY FOR INCOMING WATER VAPOR
...
! TIME-VARYING ATMOSPHERIC PARAMETERS
     ...
     REAL, DIMENSION(KTS:KTE) ::   RHOA  ! AREA WEIGHTED AIR DENSITY
...
! DUMMY SIZE DISTRIBUTION PARAMETERS
        ...
        REAL DLAMS,DLAMR,DLAMI,DLAMC,DLAMG,LAMMAX,LAMMIN,qtot_ini,qtot_ten,DUM_ini,DUM_end,strtmass,endmass
...
!++jtb
            !RHOA = AR3D * PRES/(R*T3D)
            RHOA = AR3D * RHOCL
            T3D_IN  = T3D
            QV3D_IN = QV3D


           strtmass = sum( DZQ*RHOA*(qv3d+qc3d+qi3d+qr3d+qni3d+qg3d ), 1 )
        qtot_ini = sum( qv3d + qc3d +qi3d + qni3d + qr3d + qg3d , 1)
             write(*,*) " Morr start mass ",strtmass
!--jtb
...
#else
            SIMPLE_STUPID = .TRUE.
            IF ( SIMPLE_STUPID ) THEN
                           ! Simple_stupid means liquid phase is only game in town
                           ! Liquid phase mu-phys are about 1000000 times simpler
#endif
!--jtb
...
!++jtb
        qtot_ten  = sum( qv3dten + qc3dten +qi3dten + qni3dten + qr3dten + qg3dten , 1)
           !write(*,*) " MOrr uphys check",qtot_ten,qtot_ini
!--jtb
...
!++jtb (Change RHO in Sedimentation Calc to RHOA )
      DO K = KTS,KTE
      !!RHOA(K) = RHO(K)*AR3D(K)
      RHOA(K) = RHOCL(K)*AR3D(K)
      END DO
!--jtb
      ...
      DO K = KTE,KTS,-1
        ...
        DUMR(K) = QR3D(K) !+QR3DTEN(K)*DT  ! ++ dbg jtb
...
! MULTIPLY VARIABLES BY RHO
      DUMR(k) = DUMR(k)*RHOA(K)  !++jtb
      DUMI(k) = DUMI(k)*RHOA(K)  !++jtb
      DUMFNI(k) = DUMFNI(K)*RHOA(K)  !++jtb
      DUMQS(k) = DUMQS(K)*RHOA(K)  !++jtb
      DUMFNS(k) = DUMFNS(K)*RHOA(K)  !++jtb
      DUMFNR(k) = DUMFNR(K)*RHOA(K)  !++jtb
      DUMC(k) = DUMC(K)*RHOA(K)  !++jtb
      DUMFNC(k) = DUMFNC(K)*RHOA(K)  !++jtb
      DUMG(k) = DUMG(K)*RHOA(K)  !++jtb
      DUMFNG(k) = DUMFNG(K)*RHOA(K)  !++jtb
...
! invert height index
     do K=KTS,KTE
       LI      = KTE - K + KTS ! LP+1-L
       if (allocated( PCEPHYS%RF )) PCEPHYS%RF(Li,IPC) = FR(K)
       if (allocated( PCEPHYS%SF )) PCEPHYS%SF(Li,IPC) = FS(K)
       if (allocated( PCEPHYS%HF )) PCEPHYS%HF(Li,IPC) = FG(K)
     end do
     
     DUM_ini = sum( (RHOA*QV3d + DUMR + DUMQS + DUMI + DUMC + DUMG)*DZQ , 1 )
!--jtb
...
! ADD FALLOUT TERMS TO EULERIAN TENDENCIES
      ...
      QRSTEN(K) = QRSTEN(K)-FALTNDR/NSTEP/RHOA(k) !++jtb
      QISTEN(K) = QISTEN(K)-FALTNDI/NSTEP/RHOA(k) !++jtb
      NI3DTEN(K) = NI3DTEN(K)-FALTNDNI/NSTEP/RHOA(k) !++jtb
      QNISTEN(K) = QNISTEN(K)-FALTNDS/NSTEP/RHOA(k) !++jtb
      NS3DTEN(K) = NS3DTEN(K)-FALTNDNS/NSTEP/RHOA(k) !++jtb
      NR3DTEN(K) = NR3DTEN(K)-FALTNDNR/NSTEP/RHOA(k) !++jtb
      QCSTEN(K) = QCSTEN(K)-FALTNDC/NSTEP/RHOA(k) !++jtb
      NC3DTEN(K) = NC3DTEN(K)-FALTNDNC/NSTEP/RHOA(k) !++jtb
      QGSTEN(K) = QGSTEN(K)-FALTNDG/NSTEP/RHOA(k) !++jtb
      NG3DTEN(K) = NG3DTEN(K)-FALTNDNG/NSTEP/RHOA(k) !++jtb
...
! OF LIQUID WATER CANCELS THIS FACTOR OF 1000
      ...
      write(*,*) " Morr fall SUBstep   ",NSTEP


!++jtb
     DUM_end = sum(  (RHOA*QV3d + DUMR + DUMQS + DUMI + DUMC + DUMG)*DZQ , 1 )
        !write(*,*) " Morr Sed check  ",DUM_ini,DUM_end,DUM_ini-DUM_end
!--jtb
...
!++jtb (if 0 turns off sedimentation tends for masses)
#if 1
...
!++jtb
           endmass = sum( DZQ*RHOA*(qv3d+qc3d+qi3d+qr3d+qni3d+qg3d ), 1 )
             write(*,*) " Morr end mass   ",endmass
             write(*,*) " Morr mass chng  ",endmass-strtmass
             write(*,*) " Morr precip     ",precrt+snowrt,precrt
!--jtb


        T3D_tt = (T3D-T3D_in)/DT

        call MORR_UPHYS_DIAGS( PRACS, T3D_tt , T3D_in, QV3D_in, QVS, QVI, IPC )
