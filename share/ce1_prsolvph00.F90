...
! 
!-----------------------------
MODULE CE1_PRSOLVPH00

!------------------------------------
! Do not use Davies-Jones separation
! into hydro and nonhydro components
!------------------------------------
! Created from prsolvph2 07/28/16 
!------------------------------------
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  SUBROUTINE PSOLVPH( PCE, PCECLST, PP, BYC, PHYD, DT )
   ...
   INTEGER           ::  LP, L
      ...
      CALL ZMOM_CE1    (  PCE, BYC, PP , ABELL, DT )
...
clb=CONTROL % BUOY_CONDLOAD
   ...
   RHD   = - THD / PCECLST(myev)%THBCK ! correct anelastic expression?
   ...
   DO L=2,LP                     !
      !!RHS(L) =     (2*PI*MU_H/ABELL) * PHYD(L) *RHO(L)
      RHS(L) =     ( RHO(L-1)*BYC(L-1) -  RHO(L)*BYC(L)  )/( ZL(L-1)-ZL(L) ) / (0.5*(RHO(L-1)+RHO(L)) )
   END DO
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 SUBROUTINE ZMOM_CE1(  PCE, BYC, PP , ABELL, DT )  

   type (T_ONE_PCE)  ,       intent(inout)  ::  PCE
   REAL, DIMENSION(PCE%L)  , INTENT(in)     ::  BYC
   REAL, DIMENSION(PCE%L+1), INTENT(in)     ::  PP
   REAL,                     INTENT(in)     ::  DT, ABELL

   REAL  :: P(PCE%L+1), BY(PCE%L), PFZ(PCE%L), AX(PCE%L+1), DAZ(PCE%L), DAT(PCE%L)
   REAL  :: PPE(PCE%L+1) , ZGE(PCE%L+1)
   ...
   DAT = ( PCE%A - PCE%A_NM1) / DT
     ...
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ! Vertical gradient of pressure + buoyancy force
     !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   PFZ(1:LP) = -BYC(1:LP) + ( P(2:LP+1)-P(1:LP) ) / ( ZGE(2:LP+1)-ZGE(1:LP) )
  ...
  elsewhere
   ...
   PCE%W = PCE%W - PFZ*DT   !+ .05*DAZ*DT
               write(*,*) " DOING straight DW/Dt in prsolvph00 !!!!!!!! ********* "
#else
   ...
   elsewhere
      PCE%W = 0.
               ...
               write(*,*) " DOING FV-zmom in prsolvph00  !!!!!!!! ********* "
#endif
